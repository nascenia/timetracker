...
class Timesheet < ActiveRecord::Base
  ...
  def self.to_csv_timesheet(options2 = {})
    ...
    @user = User.all
    @time_sheets = Timesheet.all.where(project_id: options2[:project_id],
    ...
    CSV.generate(options) do |csv|
      ...
      @time_sheets.each do |timesheet|
        ...
        total_hours_spent = timesheet[:hours] + minutes_hours_spent
        ...
        csv << [timesheet[:date], resource_name, timesheet[:task],
                ...
                total_hours_spent, timesheet[:description]]
      end
    end
  end
end
