class User < ApplicationRecord
  ...
  SUPER_ADMIN_USERS = Rails.application.config.x.app[:super_admins]
  ADMIN_USERS = Rails.application.config.x.app[:admins]

  # TODO: update to enum
  # enum registration_status: %i[draft approved registered]
  # enum role: %i[employee ttf sttf]
  ...
  def self.find_for_google_oauth2(access_token, signed_in_resource=nil)
    ...
    unless user
      pre_registration = PreRegistration.where(company_email: company_email, step_no: 2).first
      if pre_registration.present?
        user = User.create(
          name: pre_registration.name,
          email: pre_registration.company_email,
          weekend_id: pre_registration.weekend_id,
          holiday_scheme_id: pre_registration.holiday_scheme_id,
          personal_email: pre_registration.personal_email,
          mobile_number: pre_registration.contact_number,
          joining_date: pre_registration.joining_date,
  ...
  def self.create_unannounced_leave
    ...
    User.active.each do |u|
      if u.approval_path.present? && u.weekend != @robi_weekend
        Rails.logger.info "#{Time.now}: Attempting unannounced leave for #{u.name}"
        ...
        unless today_entry.present? || u.has_applied_for_leave || Weekend.today?(u) || HolidayScheme.today?(u)
          ...
          Rails.logger.info "#{Time.now}: Creating unannounced leave for #{u.name}"
