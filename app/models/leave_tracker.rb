class LeaveTracker < ApplicationRecord
  ...
  def self.populate_with_commenced_date
    User.active.each do |user|
      if user.leave_tracker.present?4
        user.leave_tracker.update(:commenced_date, Time.now.at_beginning_of_year)
      end
    end
  end
  ...
  def self.populate_with_carried_forward_leave
    User.active.each do |user|
      if user.leave_tracker.present?
        ...
        user.leave_tracker.update(:carried_forward_vacation, carried_forward_vacation)
        user.leave_tracker.update(:carried_forward_medical, carried_forward_medical)
      end
    end
  end
  ...
  def self.populate_with_consumed_leave
    User.active.each do |user|
      if user.leave_tracker.present?
        user.leave_tracker.update(:consumed_vacation => 0, :consumed_medical => 0)
      end
    end
  end
